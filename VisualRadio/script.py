...
def before_script(broadcast, name, date, start_times, stt_tool_name):
    ...
    for key in sec_n:
        ...
        for idx, segment in enumerate(segments): # 각각의 sec_i.json에 대해서..
            ...
            for line in lines:
                # 시간정보 업데이트
                new_lines_sec_n.append({'time':utils.add_time(utils.format_time(time_start[idx]), line['time']), 'txt':line['txt']})
        
        # 최종 sec_n.json 생성 시작
        result_sec_n = {}

        result_sec_n['end_time'] = duration_dict[key]
...
def make_script(broadcast, name, date):
    ...
    # generate_images_by_section(broadcast, name, date, section_start)  # 이제 사용하지 않음
...
def make_script_2(file_path, save_path):
    ...
    for file in file_path:
        ...
        for text in scripts:
            logger.debug(utils.add_time(prev_end_time, text['time']))
...
# ▶▶▶ 재설계(또는 통합) 대상 2: 스크립트에서 청취자 번호 찾고, 사연 문단 인식, 키워드 추출 등.. 혼잡함
...
# ▶▶▶ 이하: 사용하지 않아도 되는 도구들 (일부 기능들을 주석처리함으로써..)
###################################### tools ###################################
